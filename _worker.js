...
let userID = CONFIG.DEFAULT_UUID;
let proxyIP = '';
let cachedUserID;
export default {
...
const routeHandler = (pathname, request, userID) => {
...
const rootRequest = request => new Response(
  JSON.stringify(request.cf),
  { headers: { 'Content-Type': 'application/json' } }
);
const userRequest = (request, userID) => {
  ...
  readableWebSocketStream.pipeTo(new WritableStream({
    async write(chunk, controller) {
      if (state.isDns && state.udpWrite) {
        for (const subdata of chunkData(chunk, CONFIG.BUFFER_SIZE)) {
          state.udpWrite(subdata);
        }
        return;
      }
      if (state.remoteSocket.value) {
        await writeToSocket(state.remoteSocket.value, chunk);
        return;
      }
      const {
        hasError,
...
const makeWebSocketStream = (webSocket, earlyHeader) => {
  let isActive = true;
  ...
  return new ReadableStream({
    start(controller) {
      webSocket.addEventListener('message', event => {
        if (!isActive) return;
        for (const chunk of chunkData(event.data, CONFIG.BUFFER_SIZE)) {
...
const writeToSocket = async (socket, data) => {
  ...
  try {
    for (const chunk of chunkData(data, CONFIG.BUFFER_SIZE)) {
...
const closeWebSocket = socket => {
  if (socket.readyState === CONFIG.WS_READY_STATE_OPEN || 
      socket.readyState === CONFIG.WS_READY_STATE_CLOSING) {
    socket.close();
  }
};
    ...
    try {
      await tcpSocket.readable.pipeTo(new WritableStream({
        async write(chunk) {
          ...
          resHeader = null;
          if (webSocket.readyState === CONFIG.WS_READY_STATE_OPEN) {
            for (const subdata of chunkData(chunkHeader, CONFIG.BUFFER_SIZE)) {
