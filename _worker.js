...
const handleWsRequest = async (request, userID, proxyIP) => {
  ...
  const writableStream = new WritableStream({
    async write(chunk) {
      ...
      responseHeader.set([passVersion[0], 0]);
...
const connectAndWrite = async (remoteSocket, address, port, rawClientData) => {
  ...
  const socket = await Promise.race([
    connect({ hostname: address, port, allowHalfOpen: true, secureTransport: "on" }),
...
const createWebSocketStream = (serverSocket, earlyDataHeader) => { 
  ...
  const handleEvent = (event) => {
    if (controllerRef) {
      switch (event.type) {
        case 'message':
          controllerRef.enqueue(event.data);
          break;
        case 'close':
          closeWebSocket(serverSocket);
          controllerRef.close();
  ...
  return new ReadableStream({
    start(controller) {
      ...
      ['message', 'close', 'error'].forEach(type => 
        serverSocket.addEventListener(type, handleEvent)
      );
    },
    cancel() {
      closeWebSocket(serverSocket);
  ...
  const writableStream = new WritableStream({
    async write(chunk, controller) {
      ...
      if (headerSent) {
        ...
        dataToSend.set(new Uint8Array(chunk), responseHeader.byteLength);
        headerSent = false;
      }
...
const closeWebSocket = (serverSocket) => {
  if ([WebSocket.OPEN, WebSocket.CLOSING].includes(serverSocket.readyState)) {
    serverSocket.close();
  }
};
...
const stringify = (arr, offset = 0) => {
  ...
  return segments.map(len => {
    const str = arr.slice(offset, offset + len).map(byte => byteToHexTable[byte]).join('');
    offset += len;
    return str;
  }).join('-').toLowerCase();
};
...
const handleUdpRequest = async (serverSocket, responseHeader) => {
  ...
  transformStream.readable.pipeTo(new WritableStream({
    async write(chunk) {
      transformStream.writable.getWriter().write(chunk).finally(writer => writer.releaseLock());
    }
  }));
