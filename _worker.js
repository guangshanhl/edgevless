...
default {
  ...
  async
  function vlessOverWSHandler(request) {
    ...
    readableWebSocketStream.pipeTo(new WritableStream({
      async write(chunk, controller) {
        ...
        if (remoteSocket.value) {
          const writer = remoteSocket.value.writable.getWriter() await writer.write(chunk);
          writer.releaseLock();
          return;
        }
        const {
  ...
  async
  function handleTCPOutBound(remoteSocket, addressRemote, portRemote, rawClientData, webSocket, vlessResponseHeader) {
    async
    ...
    async
    function tryconnect(address, port) {
    ...
    transformStream.readable.pipeTo(new WritableStream({
      async write(chunk) {
        try {
          ...
          const dataToSend = isVlessHeaderSent ? new Uint8Array([...udpSizeBuffer, ...new Uint8Array(dnsQueryResult)]) : new Uint8Array([...vlessResponseHeader, ...udpSizeBuffer, ...new Uint8Array(dnsQueryResult)]);
          if (webSocket.readyState === WebSocket.OPEN) {
