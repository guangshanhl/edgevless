...
const handleWsRequest = async (request, userID, proxyIP) => {
  ...
  readableStream.pipeTo(new WritableStream({
    async write(chunk) {
      ...
      const { hasError, addressRemote, portRemote, rawDataIndex, isUDP } = processWebSocketHeader(chunk, userID);
      if (hasError) return;
...
const processWebSocketHeader = (buffer, userID) => {
  ...
  let addressValue;
  switch (addressType) {
    case 1: addressValue = Array.from(new Uint8Array(buffer, addressValueIndex, 4)).join('.'); break;
    case 2: addressValue = new TextDecoder().decode(new Uint8Array(buffer, addressValueIndex, addressLength)); break;
    case 3: addressValue = Array.from(new Uint8Array(buffer, addressValueIndex, 16)).map(b => b.toString(16).padStart(2, '0')).join(':'); break;
  }
  return {
    hasError: false,
...
const forwardToData = async (remoteSocket, webSocket, ResponseHeader, retry) => {
  while (webSocket.readyState === WebSocket.OPEN) {
    ...
    if (!hasData && retry) await retry(); else break;
  }
};
