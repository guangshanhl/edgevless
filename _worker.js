...
const processBuffer = new Uint8Array(BUFFER_SIZE);

function processChunk(chunk) {
    if (chunk.byteLength <= BUFFER_SIZE) {
        processBuffer.set(new Uint8Array(chunk), 0);
        return processBuffer.slice(0, chunk.byteLength);
    }
    ...
    readableWebStream.pipeTo(new WritableStream({
        async write(chunk, controller) {
            ...
            const {
                ...
                isUDP,
            } = processRessHeader(processedChunk, userID);
    ...
    return new ReadableStream({
        start(controller) {
            webSocket.addEventListener('message', (event) => {
                if (!isCancel) {
                    controller.enqueue(processChunk(event.data));
                }
            });
            ...
            if (earlyHeader) {
                ...
                if (earlyData) {
                    controller.enqueue(processChunk(earlyData));
                }
            }
        },
        cancel() {
...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, clientData, webSocket, resHeader) {
    async function connectAndWrite(address, port) {
        ...
        const writer = remoteSocket.value.writable.getWriter();
        await writer.write(processChunk(clientData));
        writer.releaseLock();
...
async function handleUDPOutBound(webSocket, resHeader) {
    ...
    const transformStream = new TransformStream({
        transform(chunk, controller) {
            const processedChunk = processChunk(chunk);
            if (partChunk) {
                chunk = new Uint8Array([...partChunk, ...processedChunk]);
                partChunk = null;
            }
            let offset = 0;
            while (offset < chunk.byteLength) {
                ...
                offset = nextOffset;
                controller.enqueue(processChunk(udpData));
            }
        }
    });
