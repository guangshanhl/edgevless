...
const handleWsRequest = async (request, userID, proxyIP) => {
  ...
  webSocket.accept();
  const readableStream = createWebSocketStream(
    webSocket,
...
const createWebSocketStream = (webSocket, earlyDataHeader) => new ReadableStream({
  start(controller) {
    ...
    const handlers = {
      handleMessage: (event) => controller.enqueue(event.data),
      handleClose: () => controller.close(),
      handleError: (err) => controller.error(err)
    };
    eventHandlers.set(webSocket, handlers);
    webSocket.addEventListener('message', handlers.handleMessage);
    webSocket.addEventListener('close', handlers.handleClose);
    webSocket.addEventListener('error', handlers.handleError);
  },
  cancel() {
