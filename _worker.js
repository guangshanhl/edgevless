...
const STREAM_CONFIG = {
  READ_HIGH_WATER_MARK: 1024 * 1024,  // 1MB
  WRITE_HIGH_WATER_MARK: 1024 * 1024, // 1MB
  TRANSFORM_HIGH_WATER_MARK: 512 * 1024 // 512KB
};
...
async function ressOverWSHandler(request) {
    ...
    readableWebStream.pipeTo(new WritableStream({
        ...
        highWaterMark: STREAM_CONFIG.WRITE_HIGH_WATER_MARK,
        size(chunk) {
...
function makeWebStream(webSocket, earlyHeader) {
    ...
    return new ReadableStream({
        ...
        highWaterMark: STREAM_CONFIG.READ_HIGH_WATER_MARK
    });
}
...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, clientData, webSocket, resHeader) {
    ...
    try {
        if (!(await tryConnect(addressRemote, portRemote)) && 
            !(await tryConnect(proxyIP, portRemote))) {
            closeWebSocket(webSocket);
        }
    } catch (error) {
        closeWebSocket(webSocket);
...
async function handleUDPOutBound(webSocket, resHeader) {
    ...
    const transformStream = new TransformStream({
        ...
        highWaterMark: STREAM_CONFIG.TRANSFORM_HIGH_WATER_MARK,
    }, {
        highWaterMark: STREAM_CONFIG.TRANSFORM_HIGH_WATER_MARK,
    });
