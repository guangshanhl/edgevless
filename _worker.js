...
async function handleTCPOutBound(remoteSocket, addressRemote, portRemote, clientData, webSocket, resHeader) {
    async function connectAndWrite(address, port) {
        ...
        try {
            ...
            writer.releaseLock(); // 确保释放写锁，即使发生异常
        }
    ...
    async function handleToData(remoteSocket, webSocket, resHeader) {
        let hasData = false;
        try {
            // 使用 pipeTo 和 WritableStream 简化数据流处理
            await remoteSocket.readable.pipeTo(new WritableStream({
                async write(chunk) {
                    ...
                    if (resHeader) {
                        ...
                        resHeader = null; // 只附加一次 resHeader
                    } else {
                        bufferToSend = chunk;
                    }
            ...
            closeWebSocket(webSocket); // 错误时关闭 WebSocket
        }
        return hasData;
    }
    for (const address of [addressRemote, proxyIP]) {
        try {
            ...
            if (await handleToData(tcpSocket, webSocket, resHeader)) {
                return; // 如果成功传递数据，则结束函数
            }
        } catch (error) {
            console.error(`Connection error for address ${address}:`, error);
        }
    }
    closeWebSocket(webSocket);
}
