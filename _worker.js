...
const handleWsRequest = async (request, userID, proxyIP) => {
  ...
  const processChunk = async (chunk) => {
    ...
    if (isUDP) {
      const udpWrite = portRemote === 53 ? handleUdpRequest(webSocket, responseHeader, rawClientData) : null;
      if (udpWrite) udpWrite(chunk);
    } else {
      handleTcpRequest(webSocket, responseHeader, proxyIP, addressRemote, portRemote, rawClientData);
    }
  };
...
const handleTcpRequest = async (webSocket, responseHeader, proxyIP, addressRemote, portRemote, rawClientData) => {
...
const processSocketHeader = (buffer, userID) => {
  ...
  switch (addressType) {
    case 1:
      addressValue = Array.from(new Uint8Array(buffer, addressIndex + 1, 4)).join('.');
      break;
    case 2:
      const addressLength = view.getUint8(addressIndex + 1);
      addressValue = new TextDecoder().decode(new Uint8Array(buffer, addressIndex + 2, addressLength));
      break;
    case 3:
      addressValue = Array.from(new Uint8Array(buffer, addressIndex + 1, 16)).map(b => b.toString(16).padStart(2, '0')).join(':');
      break;
    default:
  ...
  return {
    ...
    portRemote,
    rawDataIndex: addressIndex + (addressType === 2 ? 1 + view.getUint8(addressIndex + 1) : 1),
...
const forwardToData = async (remoteSocket, webSocket, responseHeader, retry) => {
  ...
  const writable = new WritableStream({
    write: async (chunk) => {
      hasData = true;
      sendCombinedData(webSocket, responseHeader, chunk);
    }
  });
  try {
...
const sendCombinedData = (ws, header, data) => {
  const combinedLength = header.byteLength + data.byteLength;
  const combinedData = new Uint8Array(combinedLength);
  combinedData.set(header, 0);
  combinedData.set(data, header.byteLength);
...
const handleUdpRequest = async (webSocket, responseHeader, rawClientData) => {
  ...
  dnsResponses.map((response, idx) => {
      if (response) {
        sendCombinedData(webSocket, responseHeader, new Uint8Array(response));
      } else {
        closeWebSocket(webSocket);
      }
  });
