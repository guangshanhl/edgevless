...
const handleWs = async (request, userID, proxyIP) => {
  ...
  const writableStream = new WritableStream({
    async write(chunk) {
      ...
      handleTcp(remoteSocket, address, proxyIP, port, clientData, serverSocket, resHeader);
    }
  });
...
const handleTcp = async (remoteSocket, address, proxyIP, port, clientData, serverSocket, reHeader) => {
  const tryConnect = async (addr) => {
    try {
      const tcpSocket = await connectAndWrite(remoteSocket, addr, port, rawClientData);
      return await forwardToData(tcpSocket, serverSocket, resHeader);
    } catch (error) {
      return false;
    }
  };
