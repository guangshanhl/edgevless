    ...
    try {
        await readableWebSocketStream.pipeTo(new WritableStream({
            async write(chunk) {
                ...
                if (hasError) return;
                const resHeader = new Uint8Array([ressVersion, 0]);
    ...
    async function tryConnect(address, port) {
        ...
        try {
            await tcpSocket.readable.pipeTo(new WritableStream({
                async write(chunk) {
                    const chunkHeader = resHeader ? mergeUint8Arrays(resHeader, chunk) : chunk;
                    resHeader = null;
                    if (webSocket.readyState === WS_READY_STATE_OPEN) {
                            webSocket.send(chunkHeader);
                        }
                        hasData = true;
                    }
                }
...
function processRessHeader(ressBuffer, userID) {
    ...
    switch (addressType) {
        ...
        case 2:
            ...
            addressValueIndex += 1;
            addressValue = new TextDecoder().decode(new Uint8Array(ressBuffer, addressValueIndex, addressLength));
            break;
        case 3:
            addressLength = 16;
