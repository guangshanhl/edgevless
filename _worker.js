    ...
    for await (const chunk of readableWebStream) {
      if (isDns && udpWrite) {
        ...
        if (isUDP) {
          ...
          remoteSocket = await handleTCPOutBound(addressRemote, portRemote, clientData, webSocket, resHeader);
        }
      }
    }
  } catch (error) {
